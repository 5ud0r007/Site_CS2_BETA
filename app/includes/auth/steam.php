...
if (!empty($_GET["auth"]) && $_GET["auth"] == 'login') {
    ...
    try {
        ...
        if (!$openid->mode) {
            ...
            exit; // Не нужно добавлять JavaScript-редирект, если мы уже используем header
            ...
            if ($openid->validate()) {
                ...
                if (!empty($Db->db_data['LevelsRanks'])) {
                    try {
                        ...
                        if (empty($Pdox->table("")->where("steam", $steam32)->get())) {
                ...
                exit; // Убедитесь, что после header есть exit
            }
        }
    } catch (ErrorException $e) {
        // Убираем сообщения, и просто редиректим, в случае чего
        header('Location: ' . $this->General->arr_general['site']);
        exit; // Добавлено exit для избежания вывода после редиректа
    }
}
...
if (!empty($_GET["auth"]) && $_GET["auth"] == 'logout') {
    ...
    if ($this->cookieEnabled()) {
        $this->delToken($_SESSION["steamid64"]);
    }
